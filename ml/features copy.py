...
def make_label_by_category(df: pd.DataFrame):
    """
    카테고리별로 중앙값 대비 얼마나 벗어났는지(비싸도/싸도 ↑)를 0~100로 스케일링
    - apt / house-villa / officetel 각각 다른 분포를 가짐 -> 그룹별로 robust z-score
    """
    df = df.copy()
    ...
    for cat, g in df.groupby("category", dropna=False):
        ...
        if len(x) < 15:
            # 표본 너무 적으면 전체 기준으로 fallback
            # (원하면 unknown / 전체 median 기반으로 바꿔도 됨)
