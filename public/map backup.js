...
function getRadiusItems(allItems) {
  const key = MY_POS ? `${MY_POS.lat.toFixed(6)},${MY_POS.lng.toFixed(6)}` : "NO_POS";
  if (__RADIUS_ITEMS_CACHE && __RADIUS_CACHE_KEY === key) return __RADIUS_ITEMS_CACHE;

  let items = allItems;

  if (MY_POS) {
    items = filterByRadius(items, MY_POS.lat, MY_POS.lng, RADIUS_KM); // 기존 함수 사용 (정렬까지 해줌)
  } else {
    // 위치 없으면: 좌표 있는 것만 + 그냥 거리정렬 불가하니 그대로
    items = items.slice(0);
  }
...
function applyTypeLimit(radiusItems) {
  // radiusItems는 이미 거리순 정렬(현재위치 있을 때) 상태
  const type = CURRENT_TYPE;

  if (type === "all") {
    const limited = limitByCategory(radiusItems, 10); // 기존 함수 그대로
    return {
      items: limited.items,
  ...
  // ✅ 요구사항: 아파트는 40개
  // (오피스텔/빌라도 동일하게 40으로 맞추면 UX가 자연스러워서 40으로 설정)
  const maxN = 40;
...
const risk = {
  ...
  if (firstCoords) map.setCenter(firstCoords);
}
...
async function renderByCurrentFilter() {
  ...
  try {
    ...
    if (infoEl) {
      ...
      infoEl.innerHTML = `
        <p>✅ 마커 생성 완료</p>
        <p>필터: <b>${typeKo}</b> / 반경: <b>${RADIUS_KM}km</b></p>
