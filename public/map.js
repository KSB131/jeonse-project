...
let __SUPPRESS_NEXT_MAP_CLICK = false;

function suppressNextMapClick(ms = 250) {
  __SUPPRESS_NEXT_MAP_CLICK = true;
  setTimeout(() => { __SUPPRESS_NEXT_MAP_CLICK = false; }, ms);
}

kakao.maps.event.addListener(map, "click", () => {
  if (__SUPPRESS_NEXT_MAP_CLICK) return;
...
window.searchAddress = async function searchAddress() {
  ...
  // ✅ 0) 단지명/동 기반 즉시 매칭 (qNorm 선언 뒤로!)
  for (const m of markers) {
    const it = m.__item;
    if (!it) continue;

    const quickKey = normAddrKeepGu(`광주 서구 ${it.dongName ?? ""} ${it.complexName ?? ""}`);
    if (qNorm === quickKey || (!needRoad && !needJibun && quickKey.includes(qNorm))) {
...
/* =========================
   API fetch
========================= */
const DETAIL_CACHE = new Map(); // roomId -> Promise/data
...
async function fetchDabangRoomDetail(roomId) {
  ...
  const p = (async () => {
    const resp = await fetch(`/api/dabang-room-detail?room_id=${encodeURIComponent(key)}`);
    if (!resp.ok) {
...
async function ensureDatasetLoaded() {
  ...
  const [items, rt] = await Promise.all([
    fetchDataset(),
    fetchRtmsCompare(),
  ]);
  ...
  __ALL_ITEMS_CACHE = (Array.isArray(items) ? items : [])
